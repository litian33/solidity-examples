const { expect } = require("chai");
const { ethers } = require("hardhat");

describe("Sample Apps / Vault", async function () {
  const [deployer, signer1, signer2, _] = await ethers.getSigners();
  let tokenContract,vaultContract;

  beforeEach(async () => {
    const TokenContract = await ethers.getContractFactory(
      "contracts/sample_apps/10_vault/Vault.sol:AssetToken",
      deployer
    );
    tokenContract = await TokenContract.deploy();
    await tokenContract.deployed();
  
    const VaultContract = await ethers.getContractFactory(
      "contracts/sample_apps/10_vault/Vault.sol:Vault",
      deployer
    );
    vaultContract = await VaultContract.deploy(tokenContract.address);
    await vaultContract.deployed();
  
    let tx=await tokenContract.connect(deployer).mint(signer1.address, ethers.utils.parseEther("100"));
    await tx.wait();
    tx=await tokenContract
      .connect(signer1)
      .approve(vaultContract.address, ethers.utils.parseEther("100"));
      await tx.wait();
    tx=await tokenContract.connect(deployer).mint(signer2.address, ethers.utils.parseEther("100"));
    await tx.wait();
    tx=await tokenContract
      .connect(signer2)
      .approve(vaultContract.address, ethers.utils.parseEther("100"));
    await tx.wait();
  });
  
  it("is initialed deposit into the blank pool", async function () {
    let tx=await vaultContract.connect(signer1).deposit(ethers.utils.parseEther("10"));
    await tx.wait();

    expect(await tokenContract.balanceOf(vaultContract.address)).to.eq(
      ethers.utils.parseEther("10")
    );
    expect(await tokenContract.balanceOf(signer1.address)).to.eq(ethers.utils.parseEther("90"));

    expect(await vaultContract.balanceOf(signer1.address)).to.eq(ethers.utils.parseEther("10"));
    expect(await vaultContract.totalSupply()).to.eq(ethers.utils.parseEther("10"));

    expect(await sharePercentageInPool(signer1)).to.eq(100);
  });

  it("deposits from liquidity provider", async function () {
    let tx=await vaultContract.connect(signer2).deposit(ethers.utils.parseEther("20"));
    await tx.wait();

    expect(await tokenContract.balanceOf(vaultContract.address)).to.eq(
      ethers.utils.parseEther("20")
    );
    expect(await tokenContract.balanceOf(signer2.address)).to.eq(ethers.utils.parseEther("80"));

    expect(await vaultContract.balanceOf(signer2.address)).to.eq(ethers.utils.parseEther("20"));
    expect(await vaultContract.totalSupply()).to.eq(ethers.utils.parseEther("20"));

    tx=await vaultContract.connect(signer1).deposit(ethers.utils.parseEther("30"));
    await tx.wait();
    expect(await tokenContract.balanceOf(vaultContract.address)).to.eq(
      ethers.utils.parseEther("50")
    );

    expect(await sharePercentageInPool(signer1)).to.eq(60);
    expect(await sharePercentageInPool(signer2)).to.eq(40);
  });

  it("withdraws from the pool", async function () {
    let tx=await vaultContract.connect(signer1).deposit(ethers.utils.parseEther("30"));
    await tx.wait();
    tx=await vaultContract.connect(signer2).deposit(ethers.utils.parseEther("20"));
    await tx.wait();

    tx=await vaultContract.connect(signer1).withdraw(ethers.utils.parseEther("10"));
    await tx.wait();

    expect(await tokenContract.balanceOf(vaultContract.address)).to.eq(
      ethers.utils.parseEther("40")
    );
    expect(await tokenContract.balanceOf(signer1.address)).to.eq(ethers.utils.parseEther("80"));

    expect(await vaultContract.balanceOf(signer1.address)).to.eq(ethers.utils.parseEther("20"));
    expect(await vaultContract.totalSupply()).to.eq(ethers.utils.parseEther("40"));

    expect(await sharePercentageInPool(signer1)).to.eq(50);
    expect(await sharePercentageInPool(signer2)).to.eq(50);

    tx=await vaultContract.connect(signer2).withdraw(ethers.utils.parseEther("20"));
    await tx.wait();

    expect(await tokenContract.balanceOf(vaultContract.address)).to.eq(ethers.utils.parseEther("20"));
    expect(await tokenContract.balanceOf(signer2.address)).to.eq(ethers.utils.parseEther("100"));

    expect(await vaultContract.balanceOf(signer2.address)).to.eq(0);
    expect(await vaultContract.totalSupply()).to.eq(ethers.utils.parseEther("20"));

    expect(await sharePercentageInPool(signer1)).to.eq(100);
    expect(await sharePercentageInPool(signer2)).to.eq(0);
  });

  async function sharePercentageInPool(signer) {
    const lpTokenBalance = await vaultContract.balanceOf(signer.address);
    const lpTokenTotalSupply = await vaultContract.totalSupply();
    // console.log("total supply",lpTokenTotalSupply, "address",signer.address, " amount ",lpTokenBalance);
    return lpTokenBalance.mul(100).div(lpTokenTotalSupply);
  }
});
